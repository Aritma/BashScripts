...
PARAMLIST="-clear -help -version -refresh -valid"
...
#kontrola, zda není použit parametr, který ignoruje ostatní vstupy
...
if isInList "-help" $PARAMS;then
	...
	tail -63 $0
	exit
elif isInList "-version" $PARAMS;then
        [ $ARGNUM -gt 0 ] && echo "Argumenty skriptu byly ignorovány..."
        echo "Version: $VERSION"
        exit
elif isInList "-refresh" $PARAMS;then
        [ $ARGNUM -gt 0 ] && echo "Argumenty skriptu byly ignorovány..."
	[ -f $LISTNAME ] && rm $LISTNAME
        getRateList
	echo "Data aktualizována (soubor $LISTNAME)"
        exit
elif isInList "-valid" $PARAMS;then
        [ $ARGNUM -gt 0 ] && echo "Argumenty skriptu byly ignorovány..."
        getValidCurrencyList
	exit
fi
...
#pokud není zadaný žádný argument, je uživatel dotázán, zda se má zobrazit převodní tabulka
#v případě, že je zadná alespoň jeden argument, provede se pouze stažení
#pokud odmítne uživatel zobrazení tabulky, neproběhne ani stahování

if [ $ARGNUM -eq 0 ];then
	PRINTALL=""
	while true;do
		read -n1 -p 'Přejete si vypsat tabulku s kurzy měn (ANO/NE)? y/n:' PRINTALL
		[[ $PRINTALL =~ ^(y|n)$ ]] && break
	done
	if [ $PRINTALL == "y" ];then
		getRateList
...
#následující blok zpracovává vstup na základě zadaného vzoru, neplatné vzory vuhodnotí jako chybu
